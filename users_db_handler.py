...
class SqliteConnectionFactory:
    ...
    def connection(cls):
        if not cls.__connection:
            cls.__connection = sqlite3.connect(f'{Paths.data}/users.db', check_same_thread=False)
...
class UsersDataBaseHandler:
    ...
    def try_add_user(self, user):
        ...
        self.__add_user(user.id, user.first_name, user.last_name, user.phone_number)
        return True
    ...
    def add_or_update_user(self, user):
        if self.__contains_user(user.id):
            self.__update_user(user.id, user.first_name, user.last_name, user.phone_number)
        else:
            self.__add_user(user.id, user.first_name, user.last_name, user.phone_number)

    def __add_user(self, user_id, first_name = '', last_name = '', phone_number = ''):
        self.__sql.execute(f'INSERT INTO users (user_id, first_name, last_name, phone_number) VALUES ({user_id}, "{first_name}", "{last_name}", "{phone_number}")')
        self.__db.commit()
    ...
    def __update_user(self, user_id, first_name, last_name, phone_number):
        self.__sql.execute(f'UPDATE users SET first_name = "{first_name}", last_name = "{last_name}", phone_number = "{phone_number}" WHERE user_id = {user_id}')
        self.__db.commit()
